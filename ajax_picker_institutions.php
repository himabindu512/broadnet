...
require_once ('common.php');
     ...
     */
    $aColumns = array( 'I.id', 'I.name', 'IT.name', 'A.name', 'S.name' );
    ...
    $sWhere = " WHERE I.deleted=0 and I.typeId not in (2,4,5,11) ";
     }
    ...
    {
        $sWhere = " WHERE I.deleted=0 and I.typeId not in (2,4,5,11) and I.active=".$_GET['sSearch'];
    }else{
        $sWhere = " WHERE I.deleted=0 and I.typeId not in (2,4,5,11) AND (";
...
$sWhere = substr_replace( $sWhere, "", -5 );
    ...
    $sQuery = "
        ...
        FROM   $sTable WHERE I.deleted=0 and I.typeId not in (2,4,5,11)
    ";
        ...
        for ( $i=0 ; $i<count($aColumns) ; $i++ )
        {
            ...
            else if ( $aColumns[$i] != ' ' )
            {
                ...
                if($aColumns[$i]=="IT.name"){
                    ...
                    if($typeID==1){ $Spec = ' / Spec : '.getValue('name','Specialties',$specialityID);  }else{ $Spec="";} 
                    $row[] = getValue('name','institutionTypes',$typeID).$Spec;
                }elseif($aColumns[$i]=="I.name"){
                    $row[] = str_replace(',', ' ',getValue('name','institutions',$aRow['id']));
                }elseif($aColumns[$i]=="S.name"){
                    $row[] = '
                    
                    <input type="radio" name="radiotypes[]" onClick="select_institutions(\''.$aRow['id'].'\',\''.getValue('name','institutions',$aRow['id']).'\',\''.getValue('typeId','institutions',$aRow['id']).'\'),select_Specialties(\''.getValue('specialityId','institutions',$aRow['id']).'\',\''.getValue('name','Specialties',getValue('specialityId','institutions',$aRow['id'])).'\')" class="radio">';

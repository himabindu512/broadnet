...
require_once ('common.php');
     ...
     */
    $aColumns = array( 'I.id', 'I.name', 'A.name', 'I.active' );
     ...
     */
    $sWhere = " WHERE I.deleted=0 and I.typeId=3 ";
    
    if ( @$_GET['sSearch'] != "" )
    {
        if ( is_numeric($_GET['sSearch']) and (@$_GET['sSearch']==0 or @$_GET['sSearch']==1 ))
    {
        $sWhere = " WHERE I.deleted=0 and I.typeId=3 and I.active=".$_GET['sSearch'];
    }else{
        $sWhere = " WHERE I.deleted=0 and I.typeId=3 AND (";
...
$sWhere = substr_replace( $sWhere, "", -5 );
    ...
    $sQuery = "
        ...
        FROM   $sTable WHERE I.deleted=0 and I.typeId=3
    ";
    ...
    while ( $aRow = mysql_fetch_array( $rResult ) )
    {
        ...
        for ( $i=0 ; $i<count($aColumns) ; $i++ )
        {
            ...
            else if ( $aColumns[$i] != ' ' )
            {
                ...
                }elseif($aColumns[$i]=="I.active"){
                    $row[] = '
                    
                    <input type="radio" name="radiotypes[]" onClick="select_pharmacies(\''.$aRow['id'].'\',\''.getValue('name','institutions',$aRow['id']).'\')" class="radio">';

...
require_once ('common.php');
     ...
     */
    $aColumns = array( 'I.id', 'I.name', 'S.name', 'A.name', 'I.specialityId' );
     ...
     */
    $sWhere = " WHERE I.deleted=0 and I.typeId=1 ";
    
    if ( @$_GET['sSearch'] != "" )
    {
        if ( is_numeric($_GET['sSearch']) and (@$_GET['sSearch']==0 or @$_GET['sSearch']==1 ))
    {
        $sWhere = " WHERE I.deleted=0 and I.typeId=1 and I.active=".$_GET['sSearch'];
    }else{
        $sWhere = " WHERE I.deleted=0 and I.typeId=1 AND (";
...
$sWhere = substr_replace( $sWhere, "", -5 );
    ...
    $sQuery = "
        ...
        FROM   $sTable WHERE I.deleted=0 and I.typeId=1
    ";
    ...
    while ( $aRow = mysql_fetch_array( $rResult ) )
    {
        ...
        for ( $i=0 ; $i<count($aColumns) ; $i++ )
        {
            ...
            else if ( $aColumns[$i] != ' ' )
            {
                ...
                if($aColumns[$i]=="I.name"){
                ...
                }elseif($aColumns[$i]=="I.specialityId"){
                    ...
                    $row[] = '
                    <input type="radio" name="radiotypes[]" onClick="select_doctor(\''.$aRow['id'].'\',\''.str_replace(',', ' ',getValue('name','institutions',$aRow['id'])).'\',\''.getValue('name','Specialties',getValue('specialityId','institutions',$aRow['id'])).'\',\''.getValue('name','areas',$parentID).'/'.getValue('name','areas',$areaID).'\',\''.$areaID.'\')" class="radio">';

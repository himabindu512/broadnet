        ...
        <div class="row-fluid">
            <div class="span12">
                <div class="box box-bordered">
                    ...
                    <div class="box-title" >
                        <h3> Import INSPharmacy<i class="icon-th-list"></i></h3>	
                    </div>
...
{
  $data = array();
	function add_property($name,$type,$zone,$area,$address,$phone,$pharmacistName)
	{
		global $data;
		$data []= array(
        'name' => $name,
			...
			foreach ($rows as $row)
			{
				...
				{	
					...
					{ 
					  ...
					  if ( $index == 4 ) $area= $cell->nodeValue;	
                      if ( $index == 5 ) $address= $cell->nodeValue;    
                      if ( $index == 6 ) $phone= $cell->nodeValue;
                      if ( $index == 7 ) $pharmacistName= $cell->nodeValue;
					  $index += 1;
					}
					add_property($name,$type,$zone,$area,$address,$phone,$pharmacistName);
				}
				$first_row = false;
			}
	  }
...
####-----Query ---####
  ...
  { 
      if($row["name"]!=''){
          ...
          $typeID=getreqValue('id','institutionTypes','name',trim($row["type"]));
          
          
echo $user_id=$db->insert_sql("insert institutions(name,typeId,areaId,class,address,phone,pharmacistName) values('".trim($row["name"])."','$typeID','$areaId','A','".trim($row["address"])."','".trim($row["phone"])."','".trim($row["pharmacistName"])."')");
 $db->print_last_query();
